...
// Pages written for their own desktop window must fit the Shell's content column in a tab.
for (const route of ['/commander', '/settings', '/copy', '/transfers']) {
    test(`no horizontal overflow on ${route}`, async ({ page }) => {
        ...
        const metrics = await page.evaluate(() => {
            ...
            return {
                browserClass: document.documentElement.classList.contains('browser'),
        ...
        expect(metrics.browserClass).toBe(true)

...
mod tests {
    ...
    fn clear_empties_the_root_and_keeps_it() {
        ...
        std::fs::create_dir_all(data.root.join("state")).unwrap();
        std::fs::write(data.root.join("state/team.json"), "{}").unwrap();
        std::fs::write(data.root.join("storage.json"), "{}").unwrap();
        std::fs::create_dir_all(data.root.join("logs")).unwrap();
        std::fs::write(data.root.join("logs/rclone-cloud.log"), "").unwrap();
